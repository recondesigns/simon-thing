...
- **The round's clock starts on the first pad, not on Start or End round.** `startedAt` opens the round and gates the pads; `lastTapAt` is left `null` by both `start` and `logRound`, so the first tap anchors the timing. **Dot 1 therefore measures nothing and is recorded as `0`, meaning "unmeasured" rather than "instant"** — `formatDotSeconds` prints it as a dash. The gap being excluded is the machine setting up and playing its pattern, which has nothing to do with how fast anyone is tapping, and it used to be charged to the first dot of every round. Two things this leans on: a genuine interval can never be 0 (the board locks for at least `SILENT_LOCK_MS` after every tap), so 0 is a safe sentinel; and **rounds banked before this carry a real first-dot time**, so history mixes the two conventions and always will. A round's total is now first-tap-to-last-tap, which is what it should have meant.
