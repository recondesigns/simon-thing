...
**`pnpm test` runs two projects, not one.** `storybook` drives every story's play assertions in a real browser; **`unit` runs `src/**/*.test.ts` in node**, for logic with no DOM — `lib/game/cellNumbers`, `lib/speech`. Run one on its own with `npx vitest run --project unit`. Worth knowing it exists: a `.test.ts` file is collected by *nothing* if the unit project is removed, so it would go silently unrun rather than fail, and the assumption that this repo can only test through stories has already sent one issue chasing a blocker that wasn't there.
...
- `/insights` — what the history adds up to: how rounds ended, taps per pad, and how far rounds get. Built from the Figma frame `56:356` (#38).
...
`lib/insights.ts` derives everything the surface shows from `Session[]` plus the scrapped count — pure functions, unit-tested, so the shape of a chart is testable without a store, a browser or a render. Nothing is cached in the store: it is a *view* of state, and keeping it would give the app two places that could disagree about how many rounds have been played.

- **Rounds split three ways, not two — and this knowingly differs from the frame.** The frame draws completed/scrapped, but "completed" there means *banked*, and a round abandoned on the seventh dot banks exactly like one that went the full distance. Counting both as success is the opposite of what most of them were. So it is **finished / ended early / scrapped**, derived with no store change: the board forces you to end at the cap, so a banked round of `ROUND_CAP` went the distance and anything shorter did not. Ended early takes the *neutral* cream, not the danger colour — losing a round is the ordinary outcome; danger is reserved for rounds thrown away on purpose.
...
- **"Where rounds end" can only count *completed* rounds.** A scrapped round also ended somewhere, but scrapping is defined as not keeping it, so there is nothing to bin. The histogram therefore sums to `completed`, not to the `total` beside the split bar. The frame's mock shows them summing to the same number; that is not reachable and never will be.
- **Round length comes from `durations`, not `pads`**, so rounds banked before store v2 still bin correctly — they know how long they were, just not where they landed. The tap counts can't include them, so those taps are reported separately rather than folded in (which would stop the nine counts adding up) or dropped (which would under-report play).
...
- Read-back speed has four settings: Fast 350, Normal 500, Relaxed 800, Slow 1100. The slower three are even 300ms steps; **Fast is deliberately off that scale**, sitting halfway to Normal because its original 200 ran too quick to follow at the machine. Don't restore the even step for symmetry. Neither adding a setting nor retuning one needs a migration — what persists is the `cadence` key, never the milliseconds.
...
- **Numbers are read back in groups of three**, phone-number style (`GROUP_SIZE` in `lib/speech.ts`), and **the gap inside a group is a third of the gap between groups** (`INTRA_GROUP_RATIO`). The chunking is what pays for the tighter gap: nine numbers read flat have to be held as nine things and need air between each, but three groups of three is three things, and the boundary pause is what marks them off — so the sequence is both easier to follow and shorter. The four cadence settings are unchanged and still mean what they say, but they are now the *group-boundary* pause rather than a flat one, so a given setting sounds faster than it used to. That was accepted deliberately; don't restore a flat gap to make the cadences sound as before. **`INTRA_GROUP_RATIO` is a proportion, not a fixed ms**, so the cadence still governs the whole read-back rather than only its boundaries — and it is the number to turn if the triplets want tightening again, which is a judgement only the machine can settle. The grouping is where nearly all the time comes from: on a twenty-dot round it saves **68s at Relaxed**, against the ~4.5s the threshold costs.
- **`MIN_INTRA_GROUP_MS` floors the in-group gap at 160ms**, because a pure proportion breaks down at the fast end — the ratio put Fast's triplets at 117ms, which ran together at the machine. **Fix that end with the floor, never by raising the ratio:** the ratio loosens all four cadences and hands back most of the 68s to solve a problem only Fast has. The floor costs Fast ~5.6s across a twenty-dot round and leaves the other three untouched (Normal is already 167ms). The in-group gap is also clamped to never exceed the boundary gap — dead code at today's cadences, and not dead the moment anyone adds one under 160ms. **All of this is pinned by `lib/speech.test.ts`** (#71), which asserts the *structure* from `GROUP_SIZE` so a deliberate retune doesn't fail it, and the *per-cadence milliseconds* spelled out so a change to those has to be made on purpose. Each guard was checked by breaking the implementation and watching it fail: flattening the gaps fails 8, removing the floor fails exactly the 2 Fast tests, deleting the clamp fails 1.
- **`WaitingIndicator` groups its dots in threes to match** (#72), reading `GROUP_SIZE` from `lib/speech` rather than repeating a `3`. Its two gaps are in the **same 1:3 ratio** as the audio's in-group and boundary pauses — deliberately the same proportion, so the picture and the sound describe one rhythm. The grouping was made **width-neutral** (3px and 9px against the old flat 5px, 2px narrower at the twenty-dot cap) because **the indicator already overruns the screen at a full round**: 457px of content on a 390px phone, and the toast centres itself, so roughly 34px is clipped off *each* end — including the first dots, on the longest read-back, where progress matters most. That overflow is older than the grouping and is still there.
- **`scrappedRounds` is a count, not records.** A scrapped round is deliberately not kept — that is what scrapping means — but *how often* it happens is worth knowing, and it can only be known by counting at the moment of discard, since nothing downstream can infer it from what survives. Only rounds with at least one dot count; scrapping an untouched round is housekeeping. It resets with `clearHistory` and `resetApp`, because a count describing history that no longer exists would report scrapped rounds against no rounds at all. No migration was needed: a key absent from persisted state leaves the initial `0` standing.
